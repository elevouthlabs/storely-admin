...
const roles: AdminRole[] = ["Super Admin", "Ops Manager", "Finance", "Moderator", "Growth"];
...
export const EditModal = ({ isOpen, onClose, adminName, currentRole }: EditModalProps) => {
  if (!isOpen) return null;

  return (
    <div className="fixed top-[-20px] left-0 inset-0 z-[9999] bg-black/40 p-4" role="dialog" aria-modal="true">
      <div className="flex min-h-full items-center justify-center">
        <div className="w-full max-w-[320px] overflow-hidden rounded-[2px] border border-[#e6e4ea] bg-white">
          <div className="flex items-center justify-between border-b border-[#ece9f1] px-4 py-3">
            <h2 className="text-[14px] font-semibold text-[#31283e]">Edit Role: {adminName}</h2>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="space-y-4 px-4 py-4">
            <div>
              <p className="mb-1.5 text-[12px] font-medium text-[#4f4a58]">Current Role</p>
              <p className="text-[12px] text-[#827c8e]">{currentRole}</p>
            </div>

            <div>
              <label className="mb-1.5 block text-[12px] font-medium text-[#4f4a58]">New Role</label>
